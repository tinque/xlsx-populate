...
class Workbook {
    ...
    /**
     * Copy a sheet to the workbook.
     * @param {string|number} sheetNameOrIndex - The sheet name or index to copy
     * @param {string} name - The name of the sheet copy. Must be unique, less than 31 characters, and may not contain the following characters: \ / * [ ] : ?
     ...
     */
    copySheet(sheetNameOrIndex, name, indexOrBeforeSheet) {

        const sheetToCopy = this.sheet(sheetNameOrIndex);
        if (!sheetToCopy) {
            throw new Error("Unable to find the sheet to copy.");
        }

        // Validate the sheet name.
        this._validateSheetName(name);

        // Get the destination index of new sheet.
        const index = this._getDestinationIndexOfNewSheet(indexOrBeforeSheet);
        ...
        // Create the copy of the sheet.
        const sheet = new Sheet(this, sheetIdNode, sheetToCopy._node, sheetToCopy._relationships);
    ...
    /**
     * Validate the name of the sheet.
     * @param {string} name - The name of the sheet. Must be unique, less than 31 characters, and may not contain the following characters: \ / * [ ] : ?
     * @return {undefined}
     */
    _validateSheetName(name) {
        // Validate the sheet name.
     ...
     * Validate the name of the sheet.
     * @param {number|string|Sheet} [indexOrBeforeSheet] The index to move the sheet to or the sheet (or name of sheet) to move this sheet before. Omit this argument to move to the end of the workbook.
     * @return {number} Index for the new sheet
     */
    _getDestinationIndexOfNewSheet(indexOrBeforeSheet) {
        // Get the destination index of new sheet.
        let index;
        if (_.isNil(indexOrBeforeSheet)) {
